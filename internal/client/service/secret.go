...
func (s *Secret) Upload(secret dto.SecretRequest, data *dto.PlainData) error {
	masterKey, err := s.storage.Key()
	if err != nil {
		return fmt.Errorf("unauthorized, try login to system :%w", err)
	}

	secret.Data, err = ecryptData(masterKey, data)
	if err != nil {
...
func ecryptData(masterKey []byte, payload *dto.PlainData) (*dto.EncryptedData, error) {
	...
	result.Data, err = crypto.EncryptAES(key, payload.Data)
	if err != nil {
...
func deryptData(masterKey []byte, data *dto.EncryptedData) (*dto.PlainData, error) {
	var result *dto.PlainData
	...
	result.Data, err = crypto.DecryptAES(key, data.Data)
	if err != nil {
